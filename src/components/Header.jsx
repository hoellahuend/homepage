...
export default function Header({
  ...
  const handleClick = (page, path) => {
    ...
    navigate(path);
    setMenuOpen(false); // Mobile Menü schließen
  };
    ...
    >
      ...
      <div className="h-20 flex items-center justify-between px-8 relative z-50">
        ...
        <nav className="hidden md:flex space-x-8 font-tefont text-2xl text-pri">
          ...
          <button
            onClick={() => handleClick("termine", "/termine")}
            className="hover:text-sec transition bg-ter p-2 text-pri"
          >
            Termine
          </button>
          <button
            onClick={() => handleClick("verein", "/verein")}
            className="hover:text-sec transition"
          >
            Verein
          </button>
      ...
      <div
        ...
        style={{
          bottom: animating ? "-30px" : "-70px", // runter während animating
